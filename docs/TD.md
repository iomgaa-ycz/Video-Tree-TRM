...
class VideoTreeBuilder:
    ...
    def build(self, video_path: str) -> TreeIndex:
        ...
        完整构建流程:
          ...
          Step 2: L2 先行 → 稀疏代表帧 VLM 描述 + 嵌入
          Step 3: L3 向下 → 注入 L2 上下文，VLM 批量帧描述（JSON）+ 嵌入
    ...
    def _segment_video(self, video_path: str) -> List[Tuple[float, float]]:
        """
        Step 1: OpenCV 读取总时长，固定步长 l1_segment_duration 切分 L1 区间
        返回: [(start, end), ...]，最后一段可能短于步长
        """
    ...
    def _extract_frames(self, video_path: str, time_range: Tuple[float, float], fps: float) -> List[Tuple[str, float]]:
        """
        L3 专用：按 fps 密集提取帧到 {cache_dir}/frames/{stem}/
        已存在的帧文件自动跳过（缓存复用）
        Returns: [(frame_path, timestamp), ...]
        """

    def _build_l2_video(self, video_path: str, clip_range: Tuple[float, float], l2_id: str) -> L2Node:
        """
        Step 2: 在 clip 区间内均匀计算 l2_representative_frames 个时间戳，
        直接 seek 提取帧（稀疏，独立于 l3_fps）→ VLM 描述 (1-2句) → embed
        """

    def _build_l3_video(self, frames: List[Tuple[str, float]], l2_description: str, l1_i: int, l2_j: int) -> List[L3Node]:
        """
        Step 3: 注入 L2 上下文的 VLM 批量帧描述
        - 主路径: 一次 VLM 调用传全部帧，要求返回 JSON 数组（每帧1-2句）
        - 降级路径: JSON 解析失败时退回逐帧调用
        """

    def _call_vlm_batch(self, prompt: str, frame_paths: List[str], n: int, l1_i: int, l2_j: int) -> List[str]:
    ...
    def _build_l1_video(self, l2_children: List[L2Node], l1_id: str, l1_range: Tuple[float, float]) -> L1Node:
        """Step 4: 拼接 L2 描述 → vlm.chat()（纯文本）生成 2-3 句摘要 → embed"""
```
